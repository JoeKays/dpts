...
- root access on the device you access dpts from in order to add a DNS entry (on Android adb root is sufficient, on Linux you need root to modify the /etc/hosts file, see number 3 under section *Problems and Solutions* for more info).
...
9. Follow the required steps 3 and 4 under the following section to make it work.

## Problems and Solutions
If the page loads but there is an error (don't worry, this is expected) it could be either:

1. Your browser is not compatible (see section *Requirements* above).
...
3. **required**: The default DPT API domain 'digitalpaper.local' must be resolvable by DNS from your device (Try to open https://digitalpaper.local:8443 in your browser to check this. If it works, accept the certificate).

    If your device can not resolve this domain (this is expected, some error should appear) you can add a DNS entry for digitalpaper.local on your device.
    
    On the DPT you can simply add the following to /system/etc/hosts (*no need to reboot or anything*):
    ```
    127.0.0.1   digitalpaper.local
    ::1         digitalpaper.local
    ```

    *Note:* (Linux only) If you are using the dpt-forward python script (see section *USB support*) you can put the same entry on your Linux machine in /etc/hosts. This makes USB work, but at the expense that you have to use the script also when using WiFi.

    On other devices you will have to find out the DPT's IP address and use that one instead.

4. **required**: You will either run into a certificate issue: The certificate from the DPT API is not trusted by your browser (because of a bad issuing authority),

    or (additionally) your browser (Firefox) will not allow dpts to send cross-origin requests (see https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy if you are interested in the reasons for this behaviour).
...
3. Forward all network traffic you send to the loopback interface of your device on the DPT API port to the DPT's IPv6 address*.
...
*Reminder:* You have to setup a DNS entry to loopback on your device for this to work (see number 3 under *Problems and Solutions* above).
